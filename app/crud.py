...
def create_user(db: Session, user: UserSchema):
    db_user = User(roll_no=user.roll_no, first_name=user.first_name, last_name=user.last_name, email=user.email, password=user.password, mobile_no=user.mobile_no, is_verified=user.is_verified)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
